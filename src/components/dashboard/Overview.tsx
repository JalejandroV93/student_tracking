...
export function Overview({
  ...
  const { infractions, fetchInfractions, loading: infractionsLoading, error: infractionsError } = useInfractionsStore() // Get loading/error states

  // Obtener el conteo total de estudiantes
  const { data: totalStudentsCount = 0, isLoading: isLoadingStudentsCount } = useStudentsCount()
      ...
      return (
        students
          ...
          .sort((a, b) => {
            ...
            if (a.alertStatus?.level !== "critical" && b.alertStatus?.level === "critical") return 1
            if (a.alertStatus && b.alertStatus) return b.alertStatus.count - a.alertStatus.count // Higher count first
            return 0
          })
      )
    },
  ...
  const sectionStats = useMemo(() => {
    ...
    return Object.values(SECCIONES_ACADEMICAS).map((sectionName) => {
      ...
      const sectionStudentIds = new Set(sectionInfractions.map((inf) => inf.studentId))
      const sectionStudents = students.filter((student) => sectionStudentIds.has(student.id))
  ...
  const filteredTypeICounts = filteredInfractions.filter((inf) => inf.type === "Tipo I").length
  const filteredTypeIICounts = filteredInfractions.filter((inf) => inf.type === "Tipo II").length
  const filteredTypeIIICounts = filteredInfractions.filter((inf) => inf.type === "Tipo III").length
  ...
  if (infractionsError) {
    return <div className="text-destructive">Error loading infractions: {infractionsError}</div>
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-end">
        <TrimestreSelector currentTrimestre={currentTrimestre} onTrimestreChange={setCurrentTrimestre} />
      </div>
        ...
        <Card>
          ...
          <CardContent className="flex items-center justify-between pt-0">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Faltas Tipo II en este período</p>
            </div>
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-muted">
