...
import type { Student, Infraction, FollowUp, AlertSettings } from "@/types/dashboard"
...
export function DashboardContent({
  ...
  const getSection = (page: string): string | null => {
    ...
    if (["all", "preschool", "elementary", "middle", "high"].includes(section)) {
  ...
  const filteredInfractions = currentSection
    ? infractions.filter((inf) => filteredStudents.some((student) => student.id === inf.studentId))
    : infractions

  // Filtrar seguimientos basados en las infracciones filtradas
  const filteredFollowUps = currentSection
    ? followUps.filter((followUp) => filteredInfractions.some((inf) => inf.id === followUp.infractionId))
    : followUps

  // Calcular conteos para la sección actual
  const sectionTypeICounts = filteredInfractions.filter((inf) => inf.type === "I").length
  const sectionTypeIICounts = filteredInfractions.filter((inf) => inf.type === "II").length
  const sectionTypeIIICounts = filteredInfractions.filter((inf) => inf.type === "III").length
  ...
  const getPageTitle = (): string => {
    ...
    if (activePage.startsWith("student-history")) return "Historial de Estudiantes"
        ...
        {activePage === "overview" ? (
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              {currentSection !== null && <SectionSelector currentSection={currentSection} baseRoute="alerts" />}
            </div>
            <div className="text-sm text-muted-foreground">
              {currentSection ? `Mostrando alertas para ${sectionTitle}` : "Mostrando alertas para todas las secciones"}
            </div>
            <AlertsList
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <h1 className="text-3xl font-bold tracking-tight">{pageTitle}</h1>
              {currentSection !== null && (
                <SectionSelector currentSection={currentSection} baseRoute="student-history" />
              )}
            </div>
            <div className="text-sm text-muted-foreground">
              {currentSection
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <h1 className="text-3xl font-bold tracking-tight">{pageTitle}</h1>
              {currentSection !== null && (
                <SectionSelector currentSection={currentSection} baseRoute="case-management" />
              )}
            </div>
            <div className="text-sm text-muted-foreground">
              {currentSection ? `Mostrando casos para ${sectionTitle}` : "Mostrando casos para todas las secciones"}
            </div>
            <CaseManagement
              students={filteredStudents}
              infractions={filteredInfractions}
          ...
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <h1 className="text-3xl font-bold tracking-tight">{pageTitle}</h1>
              {currentSection !== null && <SectionSelector currentSection={currentSection} baseRoute="reports" />}
            </div>
            <div className="text-sm text-muted-foreground">
              {currentSection
          ...
          <div className="space-y-6">
            ...
            <Settings alertSettings={alertSettings} updateAlertSettings={updateAlertSettings} />
          </div>
        ) : null}
      </div>
    </div>
