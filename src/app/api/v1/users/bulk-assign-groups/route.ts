...
export async function POST(request: Request) {
  try {
    ...
    if (users.length !== userIds.length) {
      return NextResponse.json(
        { error: 'Algunos usuarios no existen, no son directores de grupo o ya tienen grupo asignado' },
        { status: 400 }
      );
    }

    // Verificar que el grupo existe
    const groups = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/v1/groups`, {
      headers: {
    ...
    const groupExists = availableGroups.some((g: any) => g.value === groupCode);
